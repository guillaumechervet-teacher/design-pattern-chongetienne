...
{
    [TestClass]
    public class BasketOperationCalculateBasketAmoutShould
    {
        ...
        public void ReturnCorrectAmoutGivenBasket(BasketTest basketTest) {
            var amountTotal = 0;
            foreach (var basketLineArticle in basketTest.BasketLineArticles) { // Retrive article from database
                ...
                var article = articleDatabases.First(articleDatabase => articleDatabase.Id == basketLineArticle.Id); // Calculate amount
                var amount = 0;
